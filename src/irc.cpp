...
// Copyright (c) 2011-2012 MyLibertyCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("MyLibertyCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MyLibertyCoinIRCTEST0\r");
            Send(hSocket, "WHO #MyLibertyCoinIRCTEST0\r");
        } else {
            // randomly join #MyLibertyCoinIRC00-#MyLibertyCoinIRC99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #MyLibertyCoinIRC%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MyLibertyCoinIRC%02d\r", channel_number).c_str());
        }
